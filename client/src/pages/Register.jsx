...
export default function Register() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setError(err?.response?.data?.error || 'Registracija nije uspjela.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col px-6 py-16">
      <h1 className="font-display text-2xl font-semibold text-white">Registracija</h1>
      ...
      <p className="mt-4 text-sm text-slate-400">
        Već imate račun? <Link to="/login" className="text-accentSoft hover:underline">Prijavite se</Link>
      </p>
    </div>
  );
}
